...
export function Dossie({ permissoes }: { permissoes: string[] }) {
  ...
  return (
    <Box component="section">
      ...
      <Card component="header" sx={{ mb: 2.5, p: 2.5 }}>
        ...
        >
          ...
          >
             ...
             * da macroscopia. Terceira revisão com o Hugo: o botão só aparece
             * quando o serviço do caso É necropsia; num caso de biópsia ele
             * parecia uma etapa a cumprir.
             */}
            {permissoes.includes('necropsia:executar') && dados.servico.modalidade === 'necropsia' && (
              <Button
                component={Link}
        ...
        <Card sx={{ p: 2.5 }}>
          <Stack spacing={2}>
            ...
            <IdentificacaoDoAnimal
              dados={dados}
              podeEditar={permissoes.includes('caso:corrigir_identificacao')}
              aoMudar={() => api.get<DadosDossie>(`/casos/${dados.caso.id}`).then(setDados)}
            />
      ...
      {aba === 'imagens' && id && (
        <GaleriaDoCaso casoId={id} permissoes={permissoes} moduloContexto="M05_RECEBIMENTO" tipoPadrao="recebimento" />
      )}
